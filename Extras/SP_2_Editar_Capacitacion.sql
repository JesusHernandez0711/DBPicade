...
THIS_PROC: BEGIN
    ...
    /*INSERT INTO `Capacitaciones_Participantes` (
        `Fk_Id_DatosCap`,            -- [DESTINO]: El nuevo ID de versión (v_Nuevo_Id)
        `Fk_Id_Usuario`,             -- Mismo Alumno
        `Fk_Id_CatEstPart`,          -- Mismo Estatus (Aprobado, etc)
        `PorcentajeAsistencia`,      -- Misma Asistencia acumulada
        `Calificacion`,              -- Misma Calificación actual
        `created_at`,                -- Nuevo timestamp de registro en esta versión
        `updated_at`
    )
    SELECT 
        v_Nuevo_Id,                  -- <-- INYECCIÓN DEL NUEVO PADRE
        `Fk_Id_Usuario`, 
        `Fk_Id_CatEstPart`, 
	...
	*/
		...
		-- IF v_Id_Estatus_Anterior IN (1, 2, 9, 3) THEN -- <--- ¡AQUÍ AGREGAMOS EL 3!
        ...
        SET 
            `Fk_Id_DatosCap` = v_Nuevo_Id, -- Re-apuntamos al nuevo ID
            `updated_at` = NOW()
    ...
    ELSE*/
		...
		/*INSERT INTO `Capacitaciones_Participantes` (
            ...
            `PorcentajeAsistencia`, `Calificacion`, `created_at`, `updated_at`
        )
        SELECT 
            v_Nuevo_Id,         -- Nuevo Dueño
            `Fk_Id_Usuario`,    -- Mismo Alumno
            `Fk_Id_CatEstPart`, -- Mismo Estatus
            `PorcentajeAsistencia`, 
            `Calificacion`, 
            NOW(),              -- Nueva fecha de creación para este registro
            NOW()
        FROM `Capacitaciones_Participantes`
        WHERE `Fk_Id_DatosCap` = _Id_Version_Anterior;
