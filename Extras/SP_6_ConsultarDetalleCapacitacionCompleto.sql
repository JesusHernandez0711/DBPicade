...
BEGIN
    ...
    SELECT 
        ...
        `VC`.`Id_Detalle_de_Capacitacion`  AS `Id_Detalle`,        -- ID de la versión visualizada
        `VC`.`Numero_Capacitacion`         AS `Folio`,             -- Llave de Negocio (Business Key)
        `VC`.`Clave_Gerencia_Solicitante`  AS `Gerencia_Texto`,    -- Cliente Interno (Label)
        `VC`.`Nombre_Tema`                 AS `Tema_Texto`,        -- Contenido Académico (Label)
        `VC`.`Tipo_Instruccion`            AS `Tipo_Texto`,        -- Clasificación Pedagógica (Label)
        `VC`.`Asistentes_Meta`             AS `Cupo_Programado`,   -- KPI Financiero (Integer)
