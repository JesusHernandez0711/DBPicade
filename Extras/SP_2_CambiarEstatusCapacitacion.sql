/* ====================================================================================================
   ...
   - Patrón de Diseño:    "Toggle Switch with Audit Trail Injection" (Interruptor con Inyección de Auditoría).
...
THIS_PROC: BEGIN
    ...
    /* Variable de Control de Negocio (Bandera de Seguridad) */
    ...
    IF v_Estado_Actual_Padre = 1 THEN
        ...
        /* ----------------------------------------------------------------------------------------
           ...
           Consultamos el catálogo de estatus para ver si el estado actual permite cierre (Es_Final = 1).
           ---------------------------------------------------------------------------------------- */
        ...
        IF v_Es_Estatus_Final = 0 OR v_Es_Estatus_Final IS NULL THEN
            ROLLBACK;
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'ACCIÓN DENEGADA [409]: Violación de Regla de Negocio. No se puede archivar una capacitación activa o en progreso. Debe cambiar el estatus a uno TERMINAL (Concluido/Cancelado) antes de archivar.';
        END IF;
