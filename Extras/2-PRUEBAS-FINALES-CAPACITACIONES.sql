...
CALL SP_RegistrarOrganizacion(
    'QA-CICLO-GER_2', 'GERENCIA CAPACITACIÓN QA', 
    'QA-CICLO-SUB_2', 'SUBDIRECCIÓN DESARROLLO QA', 
    'QA-CICLO-DIR_2', 'DIRECCIÓN TALENTO QA'
);
...
CALL SP_RegistrarCapacitacion(@IdAdmin, NULL, @IdGeren, @IdTema, @IdInst1, @IdSedeA, @ModPre, CURDATE(), CURDATE(), 10, @StProg, 'TEST NULL');

-- Intentar registrar sin GERENCIA
-- [ESPERADO]: 🔴 ERROR [400]: "Debe seleccionar una Gerencia válida."
CALL SP_RegistrarCapacitacion(@IdAdmin, 'QA-FAIL-01', NULL, @IdTema, @IdInst1, @IdSedeA, @ModPre, CURDATE(), CURDATE(), 10, @StProg, 'TEST NULL');

-- Intentar registrar sin FECHAS
-- [ESPERADO]: 🔴 ERROR [400]: "Las fechas de Inicio y Fin son obligatorias."
CALL SP_RegistrarCapacitacion(@IdAdmin, 'QA-FAIL-02', @IdGeren, @IdTema, @IdInst1, @IdSedeA, @ModPre, NULL, NULL, 10, @StProg, 'TEST NULL');
...
CALL SP_RegistrarCapacitacion(@IdAdmin, 'QA-FAIL-03', @IdGeren, @IdTema, @IdInst1, @IdSedeA, @ModPre, CURDATE(), CURDATE(), 2, @StProg, 'TEST CUPO');
...
CALL SP_RegistrarCapacitacion(@IdAdmin, 'QA-FAIL-04', @IdGeren, @IdTema, @IdInst1, @IdSedeA, @ModPre, '2026-12-31', '2026-01-01', 10, @StProg, 'TEST FECHAS');
...
CALL SP_RegistrarCapacitacion(@IdAdmin, 'QA-FAIL-05', @IdGeren, @IdTema, 999999, @IdSedeA, @ModPre, CURDATE(), CURDATE(), 10, @StProg, 'TEST ZOMBIE');

-- Intentar registrar con SEDE INEXISTENTE
-- [ESPERADO]: 🔴 ERROR [409]: "La Sede seleccionada no existe o está cerrada."
CALL SP_RegistrarCapacitacion(@IdAdmin, 'QA-FAIL-06', @IdGeren, @IdTema, @IdInst1, 999999, @ModPre, CURDATE(), CURDATE(), 10, @StProg, 'TEST ZOMBIE');
...
CALL SP_RegistrarCapacitacion(@IdAdmin, 'QA-DUPLICADO', @IdGeren, @IdTema, @IdInst1, @IdSedeA, @ModPre, CURDATE(), CURDATE(), 10, @StProg, 'ORIGINAL');
...
CALL SP_RegistrarCapacitacion(@IdAdmin, 'QA-DUPLICADO', @IdGeren, @IdTema, @IdInst1, @IdSedeA, @ModPre, CURDATE(), CURDATE(), 10, @StProg, 'CLON MALVADO');
...
CALL SP_RegistrarCapacitacion(
    @IdCoordinador,                                    -- _Id_Usuario_Ejecutor
...
-- Verificación: El estatus sigue siendo PROGRAMADO
SELECT 'VERIFICACIÓN: Estatus de capacitaciones después de inscripciones' AS CHECK_POINT;
SELECT dc.Id_DatosCap, c.Numero_Capacitacion, ec.Nombre AS Estatus_Actual,
       (SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap) AS Participantes_Inscritos
...
CALL SP_Editar_Capacitacion(
    @IdVerTarget, @IdCoordinador, @IdInstructor1, @IdSedeA, @IdModalPresencial, @IdEstProgramado, 
...
CALL SP_RegistrarCapacitacion(
    @IdCoord, 'QA-TEMP-ZOMBIE', @IdGeren, @IdTema, @IdInst1, @IdSedeA, @ModPre, 
    CURDATE(), CURDATE(), 5, @StProg, 'Original'
);
...
CALL SP_Editar_Capacitacion(
    @IdVersion_V1, @IdCoord, @IdInst1, @IdSedeA, @ModPre, @StProg, 
...
CALL SP_Editar_Capacitacion(
    ...
    @IdCoord, @IdInst1, @IdSedeA, @IdModalPresencial, @IdEstProgramado, 
...
CALL SP_RegistrarUsuarioPorAdmin(
    ...
    @IdRolInstructor, @IdRegimen, @IdPuesto, @IdCT, @IdDep, @IdRegion, @IdGeren, '99', 'A'
);
...
CALL SP_Editar_Capacitacion(
    ...
    @IdCoordinador,      -- _Id_Usuario_Editor (Quien autoriza)
...
/* =================================================================================
   FASE 5: AUTORIZACIÓN DE CAPACITACIONES (PROGRAMADO → POR INICIAR)
   =================================================================================
   El coordinador autoriza las capacitaciones.
   Esto cambia el estatus de PROGRAMADO a POR INICIAR.
   ================================================================================= 
...
SET `Fk_Id_CatEstCap` = @IdEstPorIniciar
WHERE `Id_DatosCap` = @IdDatosCap1;

-- Registrar en historial de cambios (si existe la tabla)
-- INSERT INTO `Historial_Cambios_Capacitacion` (...) VALUES (...);

SELECT 'Capacitación 1: PROGRAMADO → POR INICIAR' AS CAMBIO_ESTATUS;
...
UPDATE `DatosCapacitaciones` 
SET `Fk_Id_CatEstCap` = @IdEstPorIniciar
WHERE `Id_DatosCap` = @IdDatosCap2;

SELECT 'Capacitación 2: PROGRAMADO → POR INICIAR' AS CAMBIO_ESTATUS;
...
UPDATE `DatosCapacitaciones` 
SET `Fk_Id_CatEstCap` = @IdEstPorIniciar
WHERE `Id_DatosCap` = @IdDatosCap3;

SELECT 'Capacitación 3: PROGRAMADO → POR INICIAR' AS CAMBIO_ESTATUS;

-- Verificación
SELECT 'VERIFICACIÓN: Todas las capacitaciones autorizadas' AS CHECK_POINT;
SELECT dc.Id_DatosCap, c.Numero_Capacitacion, ec.Nombre AS Estatus_Actual
FROM DatosCapacitaciones dc
JOIN Capacitaciones c ON dc.Fk_Id_Capacitacion = c.Id_Capacitacion
JOIN Cat_Estatus_Capacitacion ec ON dc.Fk_Id_CatEstCap = ec.Id_CatEstCap
...
SELECT '✓ FASE 5 COMPLETADA: 3 Capacitaciones autorizadas (POR INICIAR)' AS RESULTADO;*/
...
CALL SP_Editar_Capacitacion(
    ...
    @IdCoordinador,                                    -- _Id_Usuario_Editor
...
SELECT '✓ FASE 6.9 COMPLETADA: El visor de detalles es seguro y robusto.' AS RESULTADO;

-- ---------------------------------------------------------------------------------
-- 6.5. VERIFICACIÓN DEL HISTORIAL GENERADO
-- ---------------------------------------------------------------------------------
/*SELECT '--- 6.5. Verificación del Historial Generado ---' AS LOG;

SELECT 'HISTORIAL DE VERSIONES - Capacitación 2 (debe tener 4 versiones)' AS CHECK_HISTORIAL;
SELECT 
    dc.Id_DatosCap AS Version_ID,
    dc.Activo AS Es_Vigente,
    ec.Nombre AS Estatus,
    CONCAT(ip.Nombre, ' ', ip.Apellido_Paterno) AS Instructor,
    dc.Fecha_Inicio,
    dc.Observaciones,
    dc.created_at AS Fecha_Creacion
FROM DatosCapacitaciones dc
JOIN Cat_Estatus_Capacitacion ec ON dc.Fk_Id_CatEstCap = ec.Id_CatEstCap
JOIN Usuarios u ON dc.Fk_Id_Instructor = u.Id_Usuario
JOIN Info_Personal ip ON u.Fk_Id_InfoPersonal = ip.Id_InfoPersonal
WHERE dc.Fk_Id_Capacitacion = @IdCap2
ORDER BY dc.Id_DatosCap ASC;

SELECT 'HISTORIAL DE VERSIONES - Capacitación 3 (debe tener 3 versiones)' AS CHECK_HISTORIAL;
SELECT 
    dc.Id_DatosCap AS Version_ID,
    dc.Activo AS Es_Vigente,
    ec.Nombre AS Estatus,
...
FROM DatosCapacitaciones dc
JOIN Cat_Estatus_Capacitacion ec ON dc.Fk_Id_CatEstCap = ec.Id_CatEstCap
JOIN Cat_Cases_Sedes s ON dc.Fk_Id_CatCases_Sedes = s.Id_CatCases_Sedes
JOIN Cat_Modalidad_Capacitacion m ON dc.Fk_Id_CatModalCap = m.Id_CatModalCap
WHERE dc.Fk_Id_Capacitacion = @IdCap3
ORDER BY dc.Id_DatosCap ASC;*/

-- Verificación final: Estado actual de todas las capacitaciones
/*SELECT 'VERIFICACIÓN: Estado actual de todas las capacitaciones (solo versiones vigentes)' AS CHECK_POINT;
SELECT 
    dc.Id_DatosCap, 
...
SELECT '✓ FASE 6 COMPLETADA: Cambios aplicados con historial generado' AS RESULTADO;
...
CALL SP_Editar_Capacitacion(
    ...
    @IdCoordinador,      -- Quien da el banderazo
    @CurInst, @CurSede, @CurMod,
...
/* =================================================================================
   FASE 7: EJECUCIÓN DE CAPACITACIONES (POR INICIAR → EN CURSO)
   =================================================================================
   Simulamos que llegó la fecha de inicio y las capacitaciones están en curso.
   ================================================================================= 

SELECT '═══════════════════════════════════════════════════════════════════════' AS '';
SELECT '  FASE 7: EJECUCIÓN DE CAPACITACIONES (POR INICIAR → EN CURSO)          ' AS '';
SELECT '═══════════════════════════════════════════════════════════════════════' AS '';

-- Cambiamos todas las capacitaciones a EN CURSO
UPDATE `DatosCapacitaciones` 
SET `Fk_Id_CatEstCap` = @IdEstEnCurso
WHERE `Id_DatosCap` IN (@IdDatosCap1, @IdDatosCap2, @IdDatosCap3);

-- Actualizamos el estatus de los participantes a "ASISTIÓ"
UPDATE `Capacitaciones_Participantes` 
SET `Fk_Id_CatEstPart` = @IdEstPartAsistio
WHERE `Fk_Id_DatosCap` IN (@IdDatosCap1, @IdDatosCap2, @IdDatosCap3);

SELECT 'Las 3 capacitaciones ahora están EN CURSO' AS INFO;
SELECT 'Todos los participantes marcados como ASISTIERON' AS INFO;

SELECT '✓ FASE 7 COMPLETADA: Capacitaciones en ejecución' AS RESULTADO;*/
...
CALL SP_Editar_Capacitacion(
    @IdDatosCap1, @IdCoordinador, @CurInst, @CurSede, @CurMod, 
...
CALL SP_Editar_Capacitacion(
    @IdDatosCap2, @IdCoordinador, @CurInst, @CurSede, @CurMod, 
    @IdEstEvaluacion, -- CAMBIO -> EVALUACIÓN
    @CurIni, @CurFin, 0, 'QA-CICLO: Fin de clases. Inicio de captura de calificaciones.'
);
SET @IdDatosCap2 = (SELECT MAX(Id_DatosCap) FROM DatosCapacitaciones WHERE Fk_Id_Capacitacion = @IdCap2 AND Activo = 1);
...
CALL SP_Editar_Capacitacion(
    @IdDatosCap3, @IdCoordinador, @CurInst, @CurSede, @CurMod, 
    @IdEstEvaluacion, -- CAMBIO -> EVALUACIÓN
    @CurIni, @CurFin, 0, 'QA-CICLO: Fin de clases. Inicio de captura de calificaciones.'
);
SET @IdDatosCap3 = (SELECT MAX(Id_DatosCap) FROM DatosCapacitaciones WHERE Fk_Id_Capacitacion = @IdCap3 AND Activo = 1);
...
/* =================================================================================
   FASE 8: FINALIZACIÓN Y PERÍODO DE EVALUACIÓN (EN CURSO → EVALUACIÓN)
   =================================================================================
   Simulamos que terminaron las capacitaciones y ahora están en período de evaluación.
   ================================================================================= 

SELECT '═══════════════════════════════════════════════════════════════════════' AS '';
SELECT '  FASE 8: PERÍODO DE EVALUACIÓN (EN CURSO → EVALUACIÓN)                 ' AS '';
SELECT '═══════════════════════════════════════════════════════════════════════' AS '';

-- Cambiar a estado EVALUACIÓN
UPDATE `DatosCapacitaciones` 
SET `Fk_Id_CatEstCap` = @IdEstEvaluacion
WHERE `Id_DatosCap` IN (@IdDatosCap1, @IdDatosCap2, @IdDatosCap3);

SELECT 'Las 3 capacitaciones pasaron a período de EVALUACIÓN' AS INFO;

-- ---------------------------------------------------------------------------------
-- 8.1. ASIGNAR CALIFICACIONES A PARTICIPANTES
-- ---------------------------------------------------------------------------------
SELECT '--- 8.1. Asignando calificaciones a participantes ---' AS LOG;

-- CAPACITACIÓN 1: 100% aprobados (flujo perfecto) - 10 participantes, todos aprueban
-- Calificación mínima aprobatoria: 70
UPDATE `Capacitaciones_Participantes` cp
JOIN (
    SELECT @IdPart01 AS id, 95.5 AS cal UNION ALL
    SELECT @IdPart02, 88.0 UNION ALL
    SELECT @IdPart03, 92.0 UNION ALL
    SELECT @IdPart04, 78.5 UNION ALL
    SELECT @IdPart05, 85.0 UNION ALL
    SELECT @IdPart06, 90.0 UNION ALL
    SELECT @IdPart07, 75.0 UNION ALL
    SELECT @IdPart08, 82.5 UNION ALL
    SELECT @IdPart09, 98.0 UNION ALL
    SELECT @IdPart10, 88.5
) AS califs ON cp.Fk_Id_Usuario = califs.id
SET 
    cp.Calificacion = califs.cal,
    cp.Fk_Id_CatEstPart = IF(califs.cal >= 70, @IdEstPartAprobado, @IdEstPartReprobado)
WHERE cp.Fk_Id_DatosCap = @IdDatosCap1;

SELECT 'Capacitación 1: 10/10 participantes APROBADOS (100%)' AS RESULTADO_CAP1;

-- CAPACITACIÓN 2: 75% aprobados (6 de 8) - NO ACREDITADO
UPDATE `Capacitaciones_Participantes` cp
JOIN (
    SELECT @IdPart01 AS id, 85.0 AS cal UNION ALL
    SELECT @IdPart02, 72.0 UNION ALL
    SELECT @IdPart03, 55.0 UNION ALL  -- REPROBADO
    SELECT @IdPart04, 68.0 UNION ALL  -- REPROBADO
    SELECT @IdPart05, 90.0 UNION ALL
    SELECT @IdPart06, 78.0 UNION ALL
    SELECT @IdPart07, 82.0 UNION ALL
    SELECT @IdPart08, 75.0
) AS califs ON cp.Fk_Id_Usuario = califs.id
SET 
    cp.Calificacion = califs.cal,
    cp.Fk_Id_CatEstPart = IF(califs.cal >= 70, @IdEstPartAprobado, @IdEstPartReprobado)
...
SELECT 'Capacitación 2: 6/8 participantes APROBADOS (75%) - NO ACREDITADO' AS RESULTADO_CAP2;

-- CAPACITACIÓN 3: 83.3% aprobados (5 de 6) - ACREDITADO
UPDATE `Capacitaciones_Participantes` cp
JOIN (
    SELECT @IdPart01 AS id, 92.0 AS cal UNION ALL
    SELECT @IdPart02, 88.0 UNION ALL
    SELECT @IdPart03, 65.0 UNION ALL  -- REPROBADO
    SELECT @IdPart04, 78.0 UNION ALL
    SELECT @IdPart05, 95.0 UNION ALL
    SELECT @IdPart06, 80.0
) AS califs ON cp.Fk_Id_Usuario = califs.id
SET 
    cp.Calificacion = califs.cal,
...
SELECT 'Capacitación 3: 5/6 participantes APROBADOS (83.3%) - ACREDITADO' AS RESULTADO_CAP3;

SELECT '✓ FASE 8 COMPLETADA: Evaluaciones registradas' AS RESULTADO;*/
...
-- Verificación
/*SELECT 'VERIFICACIÓN: Resultado de acreditación' AS CHECK_POINT;
SELECT 
    c.Numero_Capacitacion,
    ec.Nombre AS Estatus_Acreditacion,
    (SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap) AS Total_Participantes,
    (SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap AND Calificacion >= 70) AS Aprobados,
    ROUND((SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap AND Calificacion >= 70) * 100.0 / 
          (SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap), 1) AS Porcentaje_Aprobacion
FROM DatosCapacitaciones dc
JOIN Capacitaciones c ON dc.Fk_Id_Capacitacion = c.Id_Capacitacion
JOIN Cat_Estatus_Capacitacion ec ON dc.Fk_Id_CatEstCap = ec.Id_CatEstCap
WHERE c.Numero_Capacitacion LIKE 'QA-CICLO%';*/

SELECT '✓ FASE 9 COMPLETADA: Acreditación determinada' AS RESULTADO;
...
/* =================================================================================
   FASE 10: CIERRE DE CAPACITACIONES (ACREDITADO/NO ACREDITADO → FINALIZADO)
   =================================================================================
   El coordinador cierra las capacitaciones después de revisar las evaluaciones.
   O el sistema las cierra automáticamente después de 1 mes.
   ================================================================================= 
...
-- El coordinador cierra las capacitaciones
/*UPDATE `DatosCapacitaciones` 
SET `Fk_Id_CatEstCap` = @IdEstFinalizado
WHERE `Id_DatosCap` IN (@IdDatosCap1, @IdDatosCap2, @IdDatosCap3);

SELECT 'Las 3 capacitaciones han sido FINALIZADAS por el coordinador' AS INFO;

SELECT '✓ FASE 10 COMPLETADA: Capacitaciones cerradas' AS RESULTADO;*/
...
CALL SP_CambiarEstatusCapacitacion(NULL, @IdCoordinador, 0);
...
CALL SP_CambiarEstatusCapacitacion(@IdCap1, @IdCoordinador, 99);
...
CALL SP_CambiarEstatusCapacitacion(999999, @IdCoordinador, 0);


-- 11.0.3. VALIDACIÓN DE IDEMPOTENCIA (CAPA 3)
...
CALL SP_CambiarEstatusCapacitacion(@IdCapAlive, @IdCoordinador, 0);
...
-- Para probar esto, necesitamos un curso "VIVO" (No Finalizado).
-- Vamos a "revivir" temporalmente la Cap 2 a estado 'EN CURSO' (Id 3) solo para este test.
-- (Usamos Update directo solo para preparar el escenario de test, simulando un error de usuario).
UPDATE DatosCapacitaciones SET Fk_Id_CatEstCap = 3 WHERE Id_DatosCap = @IdDatosCap2;

-- Intentar archivar un curso EN CURSO
-- [ESPERADO]: 🔴 ERROR [409]: "ACCIÓN DENEGADA... El estatus actual es 'EN CURSO'..."
CALL SP_CambiarEstatusCapacitacion(@IdCap2, @IdCoordinador, 0);

-- Restauramos el estado correcto de la Cap 2 (FINALIZADO/NO ACREDITADO) para continuar el script
UPDATE DatosCapacitaciones SET Fk_Id_CatEstCap = @IdEstNoAcreditado WHERE Id_DatosCap = @IdDatosCap2;*/
...
CALL SP_CambiarEstatusCapacitacion(
    ...
    @IdCoordinador, -- _Id_Usuario_Ejecutor (Quien archiva)
    0
);
...
-- El coordinador archiva las capacitaciones
/*UPDATE `DatosCapacitaciones` 
SET `Fk_Id_CatEstCap` = @IdEstArchivado
WHERE `Id_DatosCap` IN (@IdDatosCap1, @IdDatosCap2, @IdDatosCap3);

SELECT 'Las 3 capacitaciones han sido ARCHIVADAS' AS INFO;

-- VERIFICACIÓN FINAL DEL CICLO DE VIDA
SELECT '═══════════════════════════════════════════════════════════════════════' AS '';
SELECT '  VERIFICACIÓN FINAL: CICLO DE VIDA COMPLETO                            ' AS '';
SELECT '═══════════════════════════════════════════════════════════════════════' AS '';
...
SELECT 
    ...
    ec.Es_Final AS Es_Estado_Final,
    CONCAT(ip.Nombre, ' ', ip.Apellido_Paterno) AS Instructor_Final,
    s.Nombre AS Sede_Final,
    m.Nombre AS Modalidad_Final,
    dc.Fecha_Inicio,
    dc.Fecha_Fin,
    (SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap) AS Total_Participantes,
    (SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap AND Calificacion >= 70) AS Aprobados,
    ROUND((SELECT AVG(Calificacion) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap), 2) AS Promedio_Calif,
    dc.Observaciones AS Historial_Cambios
FROM DatosCapacitaciones dc
JOIN Capacitaciones c ON dc.Fk_Id_Capacitacion = c.Id_Capacitacion
JOIN Cat_Temas_Capacitacion t ON c.Fk_Id_Cat_TemasCap = t.Id_Cat_TemasCap
JOIN Cat_Estatus_Capacitacion ec ON dc.Fk_Id_CatEstCap = ec.Id_CatEstCap
JOIN Usuarios u ON dc.Fk_Id_Instructor = u.Id_Usuario
JOIN Info_Personal ip ON u.Fk_Id_InfoPersonal = ip.Id_InfoPersonal
JOIN Cat_Cases_Sedes s ON dc.Fk_Id_CatCases_Sedes = s.Id_CatCases_Sedes
...
SELECT '✓ FASE 11 COMPLETADA: Capacitaciones archivadas' AS RESULTADO;*/
...
SELECT 'Capacitación registrada correctamente en estatus PROGRAMADO' AS INFO, @IdCapCancel AS ID_Padre, @IdDatosCapCancel_V1 AS ID_Version_1;


-- 12.2. EJECUCIÓN DE LA CANCELACIÓN (VÍA SP_Editar_Capacitacion)
...
SELECT 'Capacitación CANCELADA exitosamente (Historial generado)' AS INFO, @IdDatosCapCancel_V2 AS ID_Version_Cancelada;


-- 12.3. SIMULACIÓN DE ARCHIVADO AUTOMÁTICO (JOB)
...
/* =================================================================================
   FASE 12: PRUEBA DE ESCENARIO DE CANCELACIÓN
   =================================================================================
   Creamos una capacitación adicional para probar el flujo de cancelación.
   ================================================================================= */
/*
SELECT '═══════════════════════════════════════════════════════════════════════' AS '';
SELECT '  FASE 12: PRUEBA DE CANCELACIÓN                                        ' AS '';
SELECT '═══════════════════════════════════════════════════════════════════════' AS '';*/

-- Crear una capacitación que será cancelada
...
SET @IdCapCancel = LAST_INSERT_ID();
...
VALUES (
    ...
    @IdInstructor1,
    DATE_ADD(@FechaActual, INTERVAL 90 DAY),
    DATE_ADD(@FechaActual, INTERVAL 95 DAY),
    @IdSedeA,
    @IdModalPresencial,
    @IdEstProgramado,
    1,
    'QA-CICLO: Capacitación para prueba de CANCELACIÓN'
);
SET @IdDatosCapCancel = LAST_INSERT_ID();

SELECT 'Capacitación de prueba creada en estado PROGRAMADO' AS INFO;*/

-- Cancelar la capacitación
/*UPDATE `DatosCapacitaciones` 
SET 
    `Fk_Id_CatEstCap` = @IdEstCancelado,
    `Observaciones` = CONCAT(Observaciones, ' | CANCELADO: Por falta de presupuesto.')
WHERE `Id_DatosCap` = @IdDatosCapCancel;

SELECT 'Capacitación CANCELADA exitosamente' AS INFO;*/

-- Simular archivado automático después de 3 meses
/*UPDATE `DatosCapacitaciones` 
SET `Fk_Id_CatEstCap` = @IdEstArchivado
WHERE `Id_DatosCap` = @IdDatosCapCancel;

SELECT 'Capacitación cancelada → ARCHIVADA (simulando paso de 3 meses)' AS INFO;

SELECT '✓ FASE 12 COMPLETADA: Prueba de cancelación exitosa' AS RESULTADO;*/
...
AND (ec.Nombre LIKE '%FINALIZADO%' OR ec.Nombre LIKE '%CANCELADO%' OR ec.Nombre LIKE '%ARCHIVADO%');


-- 13.2. VALIDACIÓN MATEMÁTICA DE ACREDITACIÓN
...
/* =================================================================================
   FASE 13: PRUEBAS DE VALIDACIÓN DE REGLAS DE NEGOCIO
   =================================================================================
   Validamos que las reglas de negocio se cumplan correctamente.
   ================================================================================= 

SELECT '═══════════════════════════════════════════════════════════════════════' AS '';
SELECT '  FASE 13: VALIDACIÓN DE REGLAS DE NEGOCIO                              ' AS '';
SELECT '═══════════════════════════════════════════════════════════════════════' AS '';

-- 13.1. VALIDACIÓN: No se puede editar una capacitación ARCHIVADA (excepto Admin)
SELECT '--- 13.1. Validando que ARCHIVADO es estado terminal ---' AS LOG;

-- Verificar que todas las capacitaciones archivadas tienen Es_Final = 1
SELECT 
    c.Numero_Capacitacion,
    ec.Nombre AS Estatus,
    ec.Es_Final,
    CASE WHEN ec.Es_Final = 1 THEN '✓ CORRECTO' ELSE '✗ ERROR' END AS Validacion
FROM DatosCapacitaciones dc
JOIN Capacitaciones c ON dc.Fk_Id_Capacitacion = c.Id_Capacitacion
JOIN Cat_Estatus_Capacitacion ec ON dc.Fk_Id_CatEstCap = ec.Id_CatEstCap
WHERE c.Numero_Capacitacion LIKE 'QA-CICLO%'
...
-- 13.2. VALIDACIÓN: Porcentaje de acreditación
SELECT '--- 13.2. Validando cálculo de acreditación ---' AS LOG;

SELECT 
    c.Numero_Capacitacion,
    ROUND((SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap AND Calificacion >= 70) * 100.0 / 
          (SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap), 1) AS Porcentaje,
    CASE 
        WHEN (SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap AND Calificacion >= 70) * 100.0 / 
             (SELECT COUNT(*) FROM Capacitaciones_Participantes WHERE Fk_Id_DatosCap = dc.Id_DatosCap) >= 80 
        THEN 'DEBERÍA SER ACREDITADO'
        ELSE 'DEBERÍA SER NO ACREDITADO'
    END AS Expectativa
FROM DatosCapacitaciones dc
JOIN Capacitaciones c ON dc.Fk_Id_Capacitacion = c.Id_Capacitacion
WHERE c.Numero_Capacitacion LIKE 'QA-CICLO-CAP-00%';

SELECT '✓ FASE 13 COMPLETADA: Validaciones de negocio verificadas' AS RESULTADO;*/
...
CALL SP_RegistrarCapacitacion(
    @IdCoordinador, 'QA-TEMP-DEL', @IdGerencia, @IdTema1, @IdInstructor1, 
...
/* =================================================================================
   ...
   =================================================================================
   Eliminamos todos los datos de prueba para dejar la base de datos limpia.
   ================================================================================= 

SELECT '═══════════════════════════════════════════════════════════════════════' AS '';
SELECT '  FASE 14: LIMPIEZA FINAL (TEARDOWN)                                    ' AS '';
SELECT '═══════════════════════════════════════════════════════════════════════' AS '';

SET FOREIGN_KEY_CHECKS = 0;

-- ---------------------------------------------------------------------------------
-- 14.1. ELIMINAR DATOS TRANSACCIONALES
-- ---------------------------------------------------------------------------------
SELECT '--- 14.1. Eliminando datos transaccionales ---' AS LOG;
...
WHERE `Fk_Id_Capacitacion` IN (
    SELECT Id_Capacitacion FROM Capacitaciones WHERE Numero_Capacitacion LIKE 'QA-CICLO%'
);

-- Cabeceras de capacitaciones
DELETE FROM `Capacitaciones` WHERE `Numero_Capacitacion` LIKE 'QA-CICLO%';

-- ---------------------------------------------------------------------------------
-- 14.2. ELIMINAR USUARIOS DE PRUEBA
-- ---------------------------------------------------------------------------------
SELECT '--- 14.2. Eliminando usuarios de prueba ---' AS LOG;

-- Eliminar usando el SP oficial si existe, o manualmente
CALL SP_EliminarUsuarioDefinitivamente(@IdAdminMaestro, @IdUsuarioAdmin);
CALL SP_EliminarUsuarioDefinitivamente(@IdAdminMaestro, @IdUsuarioCoordinador);
CALL SP_EliminarUsuarioDefinitivamente(@IdAdminMaestro, @IdInstructor1);
CALL SP_EliminarUsuarioDefinitivamente(@IdAdminMaestro, @IdInstructor2);
...
-- ---------------------------------------------------------------------------------
-- 14.3. ELIMINAR CATÁLOGOS ACADÉMICOS
-- ---------------------------------------------------------------------------------
SELECT '--- 14.3. Eliminando catálogos académicos ---' AS LOG;
...
-- ---------------------------------------------------------------------------------
-- 14.4. ELIMINAR CATÁLOGOS DE ESTATUS
-- ---------------------------------------------------------------------------------
SELECT '--- 14.4. Eliminando catálogos de estatus ---' AS LOG;
...
-- ---------------------------------------------------------------------------------
-- 14.5. ELIMINAR MODALIDADES
-- ---------------------------------------------------------------------------------
SELECT '--- 14.5. Eliminando modalidades ---' AS LOG;
...
-- ---------------------------------------------------------------------------------
-- 14.6. ELIMINAR INFRAESTRUCTURA FÍSICA
-- ---------------------------------------------------------------------------------
SELECT '--- 14.6. Eliminando infraestructura física ---' AS LOG;
...
-- ---------------------------------------------------------------------------------
-- 14.7. ELIMINAR CATÁLOGOS RH
-- ---------------------------------------------------------------------------------
SELECT '--- 14.7. Eliminando catálogos RH ---' AS LOG;
...
-- ---------------------------------------------------------------------------------
-- 14.8. ELIMINAR ESTRUCTURA ORGANIZACIONAL
-- ---------------------------------------------------------------------------------
SELECT '--- 14.8. Eliminando estructura organizacional ---' AS LOG;
...
-- ---------------------------------------------------------------------------------
-- 14.9. ELIMINAR GEOGRAFÍA
-- ---------------------------------------------------------------------------------
SELECT '--- 14.9. Eliminando geografía ---' AS LOG;
...
SELECT '✓ FASE 14 COMPLETADA: Base de datos limpia' AS RESULTADO;*/
