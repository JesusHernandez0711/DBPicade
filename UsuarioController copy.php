...
class UsuarioController extends Controller
{
    ...
    /* ========================================================================================
       ████████████████████████████████████████████████████████████████████████████████████████
       SECCIÓN 5: CASCADAS AJAX (DROPDOWNS DEPENDIENTES)
       ████████████████████████████████████████████████████████████████████████████████████████
       ========================================================================================

       Estos endpoints alimentan los selectores que dependen de una selección previa.
       Son consumidos por JavaScript (fetch/axios) cuando el usuario selecciona un padre.

       FLUJO DE CASCADA:
       ─────────────────
       GEOGRÁFICA:     País ──▶ Estados ──▶ Municipios
       ORGANIZACIONAL: Dirección ──▶ Subdirecciones ──▶ Gerencias

       ARQUITECTURA DE SEGURIDAD (4 CAPAS):
       ────────────────────────────────────
       CAPA 1 - SP DE LECTURA:   Valida padre activo + candado jerárquico → SIGNAL si falla.
       CAPA 2 - LARAVEL AJAX:    Valida input básico (>0) + doble catch (QueryException + genérico).
       CAPA 3 - SP DE ESCRITURA:  Anti-zombie en INSERT/UPDATE → última línea antes de persistir.
       CAPA 4 - BD FÍSICA:        FOREIGN KEY + UNIQUE → barrera definitiva si todo lo demás falla.

       ======================================================================================== */

    /**
     * ESTADOS POR PAÍS (CASCADA GEOGRÁFICA - NIVEL 2)
    ...
    public function estadosPorPais(int $idPais)
    {
        ...
        try {
            ...
            return response()->json(['error' => $this->extraerMensajeSP($e->getMessage())], 422);

        } catch (\Exception $e) {
            return response()->json(['error' => 'Error interno al cargar los estados.'], 500);
        }
    }

    /**
     * MUNICIPIOS POR ESTADO (CASCADA GEOGRÁFICA - NIVEL 3)
     * SP UTILIZADO: SP_ListarMunicipiosPorEstado
     * CANDADO JERÁRQUICO: Estado Activo=1 Y País padre Activo=1
    ...
    public function municipiosPorEstado(int $idEstado)
    {
        ...
        try {
            ...
            return response()->json(['error' => 'Error interno al cargar los municipios.'], 500);
        }
    }

    /**
     * SUBDIRECCIONES POR DIRECCIÓN (CASCADA ORGANIZACIONAL - NIVEL 2)
    ...
    public function subdireccionesPorDireccion(int $idDireccion)
    {
        ...
        try {
            ...
            return response()->json(['error' => 'Error interno al cargar las subdirecciones.'], 500);
        }
    }

    /**
     * GERENCIAS POR SUBDIRECCIÓN (CASCADA ORGANIZACIONAL - NIVEL 3)
     * SP UTILIZADO: SP_ListarGerenciasPorSubdireccion
     * CANDADO JERÁRQUICO: Subdirección Activo=1 Y Dirección padre Activo=1
    ...
    public function gerenciasPorSubdireccion(int $idSubdireccion)
    {
        ...
        try {
            ...
            return response()->json(['error' => 'Error interno al cargar las gerencias.'], 500);
        }
    }

    /* ========================================================================================
       ████████████████████████████████████████████████████████████████████████████████████████
       SECCIÓN 6: MÉTODOS PRIVADOS (UTILIDADES INTERNAS)
       ████████████████████████████████████████████████████████████████████████████████████████
       ======================================================================================== */
     ...
     *   por AJAX cuando el usuario selecciona un padre (ver SECCIÓN 5: CASCADAS AJAX).
     *
    ...
    private function cargarCatalogos(): array
    {
        return [
            ...
            // Hijos: estadosPorPais() → municipiosPorEstado() [SECCIÓN 5 - AJAX]
            'paises'           => DB::select('CALL SP_ListarPaisesActivos()'),

            // ═══ CASCADA ORGANIZACIONAL (Solo nivel raíz) ═══
            // Hijos: subdireccionesPorDireccion() → gerenciasPorSubdireccion() [SECCIÓN 5 - AJAX]
